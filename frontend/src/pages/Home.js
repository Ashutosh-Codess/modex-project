...
var ShowCard_1 = require("@/components/ShowCard");
var BookingContext_1 = require("@/contexts/BookingContext");
var react_router_dom_1 = require("react-router-dom");
var genres = ['All', 'Action', 'Sci-Fi', 'Crime', 'Drama', 'Comedy', 'Horror'];
function Home() {
    var _a = (0, BookingContext_1.useBooking)(), shows = _a.shows, fetchShows = _a.fetchShows, isLoading = _a.isLoading, error = _a.error;
    var _b = (0, react_1.useState)(''), searchQuery = _b[0], setSearchQuery = _b[1];
    var _c = (0, react_1.useState)('All'), selectedGenre = _c[0], setSelectedGenre = _c[1];
    var _d = (0, react_1.useState)('date'), sortBy = _d[0], setSortBy = _d[1];
    (0, react_1.useEffect)(function () {
        fetchShows().catch(function (err) {
            console.error("Failed to fetch shows:", err);
        });
    }, [fetchShows]);
    var filteredShows = shows
        .filter(function (show) {
        var matchesSearch = show.name.toLowerCase().includes(searchQuery.toLowerCase());
        var matchesGenre = selectedGenre === 'All' || show.genre === selectedGenre;
    ...
    var _e = (0, react_1.useState)(''), selectedLocation = _e[0], setSelectedLocation = _e[1];
    return (<div style={{
            minHeight: '100vh',
      ...
      {!selectedLocation && (<div style={{
          ...
          <select value={selectedLocation} onChange={function (e) { return setSelectedLocation(e.target.value); }} style={{
                width: '100%',
      ...
      {shows.length > 0 && selectedLocation && (<div style={{
          ...
          <div style={{ flex: 1, zIndex: 2 }}>
            ...
            <react_router_dom_1.Link to={"/booking/".concat(shows[0].id)} style={{ textDecoration: 'none', display: 'inline-block' }}>
              <button style={{
                background: '#000000',
                color: 'white',
                border: 'none',
          ...
          <div style={{ width: '250px', height: '350px', flexShrink: 0 }}>
            <img src="https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?w=400&h=600&fit=crop&q=80" alt={shows[0].name} style={{
                width: '100%',
                height: '100%',
                objectFit: 'cover',
                borderRadius: '12px',
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 1rem', position: 'relative', zIndex: 10 }}>
        {/* Section Title */}
        {selectedLocation && (<div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <h2 style={{
                fontSize: '2rem',
                fontWeight: 'bold',
                marginBottom: '0.5rem',
        ...
        {selectedLocation && (<div style={{
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ position: 'relative', flex: '1', minWidth: '250px' }}>
              ...
              <input type="text" placeholder="Search movies..." value={searchQuery} onChange={function (e) { return setSearchQuery(e.target.value); }} style={{
                width: '100%',
            ...
            <select value={selectedGenre} onChange={function (e) { return setSelectedGenre(e.target.value); }} style={{
              ...
              {genres.map(function (genre) { return (<option key={genre} value={genre}>{genre}</option>); })}
            </select>

            <select value={sortBy} onChange={function (e) { return setSortBy(e.target.value); }} style={{
                padding: '0.75rem 1rem',
                border: '2px solid rgba(255,255,255,0.2)',
                borderRadius: '8px',
                fontSize: '1rem',
        ...
        {!selectedLocation ? (<div style={{
          ...
          </div>) : filteredShows.length === 0 ? (<div style={{
                textAlign: 'center',
                padding: '4rem 0',
                background: 'rgba(139, 0, 0, 0.8)',
                borderRadius: '12px',
            ...
            {shows.length === 0 && (<react_router_dom_1.Link to="/admin" style={{ textDecoration: 'none', display: 'inline-block' }}>
                <button style={{
            ...
            {filteredShows.map(function (show) { return (<ShowCard_1.ShowCard key={show.id} show={show}/>); })}
          </div>)}
      </div>
